...
def tuner(
    ...
    *,
    name: str,  # retained at the call site to make each study explicit
    samples: int,
    epochs: int,
    cfg: DictConfig,
    storage: Path,
    ...
    return Tuner.from_objective(
        ...
        config=TuningConfig(
            ...
            ray_runtime=RayRuntimeConfig(
                temp_dir=project_path(str(cfg.ray.temp_directory)).resolve()
            ),
        ),
    )
...
@hydra.main(version_base="1.3", config_path=".", config_name="physics_deeponet")
def main(cfg: DictConfig) -> None:
    ...
    def base_objective(config, train, validation, _context, report):
        return ray_trial(
            config,
            train_data=train,
    ...
    context = RunContext(
        RunPaths(output_dir), int(cfg.seed), torch.float32, torch.device("cpu")
    )
    base_tuning = tuner(
        base_objective,
        search_space,
        ...
        experiment_name="data_driven",
    )
    best_config = serializable(base_tuning.best_config)

    def physics_objective(config, train, validation, _context, report):
        return ray_trial(
            config,
            train_data=train,
            validation_data=validation,
    ...
    physics_tuning = tuner(
        ...
        context,
        storage_path=storage,
        experiment_name="physics_weight",
    )
    physics_weight = float(physics_tuning.best_config["physics_weight"])

    best_summary = {
        "shared_model": best_config,
        "data_driven_validation_relative_l2": float(base_tuning.best_metrics[METRIC]),
        "physics_weight": physics_weight,
        "physics_informed_validation_relative_l2": float(
            physics_tuning.best_metrics[METRIC]
        ),
    }
